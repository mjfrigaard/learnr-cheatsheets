...
[Unix](https://en.wikipedia.org/wiki/Unix) refers to a family of operating systems that serve as a 'backbone' for Linux (Android, Chrome OS) and Darwin (macOS, iOS, watchOS, tvOS, audioOS, iPadOS). You use Unix on most of the software you interact with on the internet, in phones and tablets, and it's in *some* desktop computers. 

Microsoft Windows isn't based in Unix, but it's possible to [install a Linux environment](https://docs.microsoft.com/en-us/windows/wsl/about) (although not necessary if you're using RStudio on your local machine). 
...
```{r pwd-local-01.png, echo=FALSE, eval=TRUE, fig.align='center', out.height='100%', out.width='100%', fig.cap="~/projects/"}
knitr::include_graphics(path = "www/pwd-local-01.png")
```

2. If I change my working directory with `..`, I am in the home directory `~`.

```{r pwd-local-02.png, echo=FALSE, eval=TRUE, fig.align='center', out.height='100%', out.width='100%', fig.cap="~/"}
...
```{r pwd-local-03.png, echo=FALSE, eval=TRUE, fig.align='center', out.height='100%', out.width='100%', fig.cap="Still ~/"}
...
```{bash list-files-dirs, exercise=TRUE,  exercise.lines = 6}
...
The `touch` command will create a file if it doesn't exist. If the file does exist, `touch` will change the file timestamps. We can check the timestamps with `stat`

```{bash stash-txt-error, exercise = TRUE}
# enter 'stat stash.txt' and click Run Code

```

Why did we get an error? This is because in the `learnr` environment, each code chunk is it's own little self-contained environment. This means commands in previous chunks won't carry over to successive chunks (which makes it much safer to practice our command-line skills!)
...
Change: 2022-01-04 09:48:19.276836284 +0000
 ...
 We can see this file has identical dates and times for `Access`, `Modify`, and `Change`. 
